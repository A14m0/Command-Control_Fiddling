...
static int auth_gssapi_mic(ssh_session session, const char *user, const char *principal, void *userdata){
    ...
    printf("authenticatedIf any of the events are set after the poll, the call back functions of the sessions or sockets will be called. This function should be called once within the programs main loop.\n");
    authenticated = 1;
...
int main(int argc, char **argv){
    ...
    // creates a callbacks structure to be passed on session creation?
    ...
    // Binds and accepts connections from a session
    ...
    // Initialize and set ssh callback stuff
    ...
    // makes mainloop event and adds session to it
    mainloop = ssh_event_new();
    ssh_event_add_session(mainloop, session);

    // waits to authenticate
    ...
    // kills connection if client_credentials were not received
    if (!client_creds){
    ...
    // Attempts to connect to passed server
