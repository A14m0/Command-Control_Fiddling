...
const char *keyfile1 = "/etc/ssh/id_rsa.pub";
const char *keyfile2 = "/etc/ssh/id_rsa";
const char *username = "aris";
const char *password = "lala";
 
const char *server_ip = "127.0.0.1";
 
const char *remote_listenhost = "localhost"; /* resolved by the server */ 
int remote_wantport = 80;
...
int main(int argc, char *argv[])
{
    ...
    sock = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP); // PF_INET => Protocol family socket
    if(sock == -1) {
        perror("socket");
        return -1;
    }
 
    sin.sin_family = AF_INET; // then sets it to address family socket lol
